...
wine64_bin = '/usr/local/wine-proton-experimental/bin/wine64'
wine32_bin = File.join(I386_PKG_ROOT, 'usr/local/wine-proton-experimental/bin/wine')

if !File.exist?(wine64_bin)
  perr "#{wine64_bin} doesn't exist!"
  perr "Install emulators/wine-proton-experimental first."
  exit(1)
end
...
raise if !File.exist?('/usr/local/wine-proton-experimental/lib/wine/x86_64-windows')
raise if !File.exist?(File.join(I386_PKG_ROOT, 'usr/local/wine-proton-experimental/lib/wine/i386-windows'))
  ...
  def set_setup_state(state)
    if state != $setup_state
      case state
        ...
        when :manifest
          pwarn "Registering emulators/wine-proton-experimental as a compatibility tool..."
        when :done
      ...
      Dir.chdir("proton_#{PROTON_VERSION}") do
        ...
        set_up_file('files') do |target_dir|
          ...
          Dir.chdir(File.join("#{target_dir}.tmp", 'lib/wine/i386-windows')) do
            for file in Dir[File.join(I386_PKG_ROOT, 'usr/local/wine-proton-experimental/lib/wine/i386-windows/*.{cpl,dll,drv,exe,ocx}')]
          ...
          Dir.chdir(File.join("#{target_dir}.tmp", "#{PROTON_VERSION.to_i < 10 ? 'lib64' : 'lib'}/wine/x86_64-windows")) do
            for file in Dir['/usr/local/wine-proton-experimental/lib/wine/x86_64-windows/*.{cpl,dll,drv,exe,ocx}']
              if !File.exist?(File.basename(file))
                set_setup_state(:symlinks)
                safe_system('ln', '-s', file)
              end
          ...
          set_up_file(File.join("#{target_dir}.tmp", 'bin')) do |target|
            ...
            FileUtils.mkdir_p(target)
            Dir.chdir(target) do
              for file in Dir['/usr/local/wine-proton-experimental/bin/{msidb,wine,wine64,wineserver}']
                safe_system('ln', '-s', file)
              end
            end
          end
          ...
          if PROTON_VERSION.to_i >= 10
            set_up_file(File.join("#{target_dir}.tmp", 'bin-wow64')) do |target|
              ...
              FileUtils.mkdir_p(target)
              Dir.chdir(target) do
                for file in Dir['/usr/local/wine-proton-experimental/bin-wow64/{msidb,wine,wineserver}']
                  safe_system('ln', '-s', file)
                end
              end
            end
